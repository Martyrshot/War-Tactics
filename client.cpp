...
void
cleanMenu(MENU *menu, ITEM **menu_items)
{
	if (NULL != menu_items)
	{
	...
	selectAction:
		...
		switch (action)
		{
			case 0:
			{
				...
				vector<vector<uint8_t>> points = getAllPathPlacementOptions(board, playerID);
				system("clear");
				printOpponentsHand(oppHandSize);
				healths = interface.getHqHealth();
				// TODO confirm which health is which
			...
			case 2:
			{
				...
				points = getPossibleMovementOptionsForUnit(board, source, playerID);
				system("clear");
				printOpponentsHand(oppHandSize);
				healths = interface.getHqHealth();
				// TODO confirm which health is which
			...
			case 3:
			{
				...
				points = getPossibleAttackOptionsForUnit(board, playerID, source);
				system("clear");
				printOpponentsHand(oppHandSize);
				healths = interface.getHqHealth();
				// TODO confirm which health is which
...
vector<uint8_t>
buildHand(vector<uint8_t> handSeeds)
{
