...
void
printHorizontalBar(int row, vector<vector<uint8_t>> points)
{
	int curTile = -1;
	bool oneMore = false;
	for (int i = 0; i < 41; i++)
	{
		if (i % 4 == 0)
		{
			...
			for (size_t j = 0; j < points.size(); j++)
			{
				...
				else if (row != 0 && points[j][1] == row - 1 && points[j][0] == curTile)
				{
					foundTile = true;
					oneMore = true;
					cout << "\033[1;33m";
...
void
printBoard(vector<vector<vector<uint8_t>>> board, int playerID,
	...
	vector<uint8_t> unit, uint8_t playerID)
{
	...
	if (board[2][x][y] == playerID && (board[1][x][y] != STATE_HQ_AND_UNIT || board[1][x][y] != STATE_PATH_AND_UNIT))
	{
		return points;
	}
	if (x < 9 && (board[1][x + 1][y] == STATE_PATH || board[1][x + 1][y] == STATE_HQ))
	{
		points.push_back({ static_cast<unsigned char>(x + 1), y });
	}
	if (x > 0 && (board[1][x - 1][y] == STATE_PATH || board[1][x - 1][y] == STATE_HQ))
	{
		points.push_back({ static_cast<unsigned char>(x - 1), y });
	}
	if (y > 0 && (board[1][x][y - 1] == STATE_PATH || board[1][x][y - 1] == STATE_HQ))
	{
		points.push_back({ x, static_cast<unsigned char>(y - 1) });
	}
	if (y < 8 && (board[1][x][y + 1] == STATE_PATH || board[1][x][y + 1] == STATE_HQ))
	{
		points.push_back({ x, static_cast<unsigned char>(y + 1) });
	}
	...
	int playerID, vector<uint8_t> unit)
{
	...
	uint8_t y = unit[1];
	vector<vector<uint8_t>> points;
	if (x < 9 && (board[1][x + 1][y] == STATE_HQ_AND_UNIT || board[1][x + 1][y] == STATE_PATH_AND_UNIT || board[1][x + 1][y] == STATE_HQ)
		&& board[2][x + 1][y] != playerID)
	{
		points.push_back({ static_cast<unsigned char>(x + 1), y });
	}
	if (x > 0 && (board[1][x - 1][y] == STATE_HQ_AND_UNIT || board[1][x - 1][y] == STATE_PATH_AND_UNIT || board[1][x - 1][y] == STATE_HQ)
		&& board[2][x - 1][y] != playerID)
	{
		points.push_back({ static_cast<unsigned char>(x - 1), y });
	}
	if (y > 0 && (board[1][x][y - 1] == STATE_HQ_AND_UNIT || board[1][x][y - 1] == STATE_PATH_AND_UNIT || board[1][x][y - 1] == STATE_HQ)
		&& board[2][x][y - 1] != playerID)
	{
		points.push_back({ x, static_cast<unsigned char>(y - 1) });
	}
	if (y < 8 && (board[1][x][y + 1] == STATE_HQ_AND_UNIT || board[1][x][y + 1] == STATE_PATH_AND_UNIT || board[1][x][y + 1] == STATE_HQ)
		&& board[2][x][y + 1] != playerID)
	{
		points.push_back({ x, static_cast<unsigned char>(y + 1) });
	}
...
vector<vector<uint8_t>>
getPossibleHQLocations(uint8_t playerID)
{
	vector<vector<uint8_t>> points;
	if (playerID == 2)
	{
